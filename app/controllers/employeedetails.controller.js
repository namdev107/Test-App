...
async function getAllEmployees(req, res) {
  try {
    ...
    if (result.length > 0) {
      const combinData = result.map((user) => ({
        id: user.id,
        empid: user.empid,
      ...
      // console.log("combinData ===========>>>>>>>>>>>>", combinData);

      const json2csv = require("json2csv").parse;
      const csvData = json2csv(combinData); // Create Csv File.

      res.setHeader(
        "Content-disposition",
        "attachment; filename=Employee_details.csv"
      );
...
async function createEmpDataPDF(req, res) {
  try {
    let where = {};
    if (req.body.empid) {
      where.empid = req.body.empid;
      ...
      employeedetails
        ...
        .catch((error) => {
          res.status(500).send({
            message:
              error.message ||
              "Some error occurred while adding the Employee Details.",
          });
        });
    }
  } catch (error) {
